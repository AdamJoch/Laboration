...
long CPJRegistry::Profile(LPCTSTR lpszEntry, COleDateTime& DateTime)
{
	...
	{
		strTemp = DateTime.Format("%I:%M:%S %p %b %d, %Y");
		lResult = Profile(lpszEntry, strTemp);
	}
	return lResult;
}
...
long CPJRegistry::Profile(LPCTSTR lpszEntry, CPoint& PointValue)
{
	...
	if (m_bGet)
	{
		...
		if ((lResult == ERROR_SUCCESS) && !strTemp.IsEmpty())
		{
			lpszToken = strtok( strTemp.GetBuffer(strTemp.GetLength()), cSeps );
			if (lpszToken != NULL)
			{
				lpEnd = lpszToken + 100;
				PointValue.x = strtol(lpszToken,&lpEnd,10);
				lpszToken = strtok( NULL, cSeps );
				if (lpszToken != NULL)
				{
					lpEnd = lpszToken + 100;
					PointValue.y = strtol(lpszToken,&lpEnd,10);
				}
			}
		}
	}
	...
	{
		strTemp.Format("%ld, %ld", PointValue.x, PointValue.y);
		lResult = Profile(lpszEntry, strTemp);
	}
	return lResult;
}
...
long CPJRegistry::Profile(LPCTSTR lpszEntry, CSize& Size)
{
	...
	if (m_bGet)
	{
		...
		if ((lResult == ERROR_SUCCESS) && !strTemp.IsEmpty())
		{
			...
			{
				lpEnd = lpszToken + 100;
				Size.cx = strtol(lpszToken,&lpEnd,10);
				lpszToken = strtok( NULL, cSeps );
				if (lpszToken != NULL)
				{
					lpEnd = lpszToken + 100;
					Size.cy = strtol(lpszToken,&lpEnd,10);
				}
			}
		}
	}
	...
	{
		strTemp.Format("%ld, %ld", Size.cx, Size.cy);
		lResult = Profile(lpszEntry, strTemp);
	}
	return lResult;
}
...
long CPJRegistry::Profile(LPCTSTR lpszEntry, CRect& rcValue)
{
	...
	if (m_bGet)
	{
		...
		if ((lResult == ERROR_SUCCESS) && !strTemp.IsEmpty())
		{
			...
			{
				lpEnd = lpszToken + 100;
				long lTemp = strtol(lpszToken,&lpEnd,10);
				switch (i)
				{
	...
	{
		strTemp.Format("%ld, %ld, %ld, %ld",rcValue.left,rcValue.top,rcValue.right,rcValue.bottom);
		lResult = Profile(lpszEntry, strTemp);
	}
	return lResult;
}
